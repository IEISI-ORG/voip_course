# Module 16 — Monitoring, Observability & Incident Response
...
- Detect the Module 14 attack signatures automatically.
...
- **Detection engineering:** turn each M14 signature into a rule (Wazuh/Loki/HOMER queries):
...
- Wazuh rules + Alertmanager routes for the M14 signatures; test each alert fires.
- A CDR analytics job feeding fraud alerts (from M15) into the same alerting pipeline.

## 4. Attack / Defend
- Run the M14 assessment again "blind"; prove the SOC view detects each phase in real time.
...
- **Lab 15.2:** Detection rules for all M14 signatures; show each firing during a replay.
...
- **PCI/lawful-intercept:** log every access with actor + reason; verify DTMF-suppressed
  segments (built in M15) actually contain no card data before archival.
...
The honeypot from M15 becomes an active-defense feed when its hits drive automated blocking
across the platform.
...
- **Lab hook (BF15 — Suricata IDS):** add a *signature* sensor beside the *behavioural* honeypot.
  ...
  Wazuh correlation. Diverse detection, one response path — the M14→detect→M16 arc.
