...
**One-liner:** Turn the Module 14 findings into a hardened platform and stop toll fraud before it
drains a budget. **Est. time:** 6h · **Prereqs:** Module 14.
...
- **Defense-in-depth layers:** network edge (nftables/geo/rate) → SBC (topology hiding, admission,
  ...
  (voicemail/feature hardening) → data (secrets, recordings, CDR) → monitoring (M16).
...
- **Lab hook:** record a call with DTMF masked during a "payment" segment; confirm no PAN is
  present and every access is logged. Audit/monitoring side continues in M16.
...
- **Lab hook (adds BF12):** deploy the honeypot, scan it, and verify the scanner IP lands in
  the `nftables` set and is refused at the real listener. Aggregation/active-response in M16.
