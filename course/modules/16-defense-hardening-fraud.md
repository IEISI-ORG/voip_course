...
**One-liner:** Turn the Module 15 findings into a hardened platform and stop toll fraud before it
drains a budget. **Est. time:** 6h · **Prereqs:** Module 15.
...
- Automate brute-force/scan defense and validate it against Module 15 attacks.
...
- **Defense-in-depth layers:** network edge (nftables/geo/rate) → SBC (topology hiding, admission,
  ...
  (voicemail/feature hardening) → data (secrets, recordings, CDR) → monitoring (M17).
...
## 2. Packet Reality
- Re-run Module 15 attack signatures and show them being dropped/banned/throttled in captures
...
- Replay every Module 15 finding; confirm remediation; record residual risk in the threat model.
...
- **Lab hook:** record a call with DTMF masked during a "payment" segment; confirm no PAN is
  present and every access is logged. Audit/monitoring side continues in M17.
...
- **Lab hook (adds BF12):** deploy the honeypot, scan it, and verify the scanner IP lands in
  the `nftables` set and is refused at the real listener. Aggregation/active-response in M17.
