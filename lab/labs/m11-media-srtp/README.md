# Lab M11 — Media Security: SRTP, DTLS-SRTP & ZRTP

**Module:** [M11](../../../course/modules/11-media-security-srtp.md) · **Est.** 5h ·
**Prereqs:** M10 (SDES keys ride TLS-protected signaling).

Goal: encrypt the media plane and get key exchange right — "encrypted" vs "actually private."

## Auto-graded foundation
```bash
bash labs/m11-media-srtp/verify.sh          # media anchor on-path + SIP-TLS present
```
Fail-closed: asserts SRTP can be anchored (rtpengine) and SDES keys can be protected (TLS from
M10). Media-payload encryption itself is capture-graded.
...
the RTP payload).
```bash
bash labs/m11-media-srtp/srtp-offer.sh 172.28.10.30       # observe RTP/SAVP + a=crypto
```
...
```bash
bash labs/m11-media-srtp/srtp-offer.sh 172.28.10.30 --strip    # downgrade attempt (no a=crypto)
```
