#!/usr/bin/env bash
# VoIPSec M17 acceptance test — alert rules are well-formed and cover the M15 threats (offline,
# deterministic, fail-closed). promtool is used if present; else a YAML parse.
# Run from lab/:  bash labs/m17-monitoring-ir/verify.sh
set -u
cd "$(dirname "$0")/../.." || exit 3
DIR=labs/m17-monitoring-ir
...
echo "== 3. detection signatures cover the M15 threats =="
