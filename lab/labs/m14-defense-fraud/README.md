# Lab M14 — Defense, Hardening & Fraud Prevention

**Module:** [M14](../../../course/modules/14-defense-hardening-fraud.md) · **Est.** 6h ·
**Prereqs:** M13.

Goal: turn the M13 findings into a hardened platform and stop toll fraud before it drains a budget.

## Auto-graded core
```bash
bash labs/m14-defense-fraud/verify.sh          # fraud detector + hardening checklist (offline)
bash labs/m14-defense-fraud/fraud-detect.sh    # scan the sample CDR
```
...
```bash
bash labs/m14-defense-fraud/fraud-detect.sh labs/m14-defense-fraud/sample-cdr.csv
CAP=100 PREFIX_CAP=50 bash labs/m14-defense-fraud/fraud-detect.sh your-cdr.csv
```
...
Re-run the M13 assessment (`labs/m13-offensive/`) against the hardened platform and produce a
...
> Layered fraud defense: prevent (allowlist + auth, M9/M12), **cap** (bounds the loss), detect +
