#!/usr/bin/env bash
# VoIPSec M18 acceptance test — the automation/CI is valid and the repo passes its own lint
# (offline, deterministic, fail-closed). Mirrors what CI runs on every change.
# Run from lab/:  bash labs/m18-testing-cloud/verify.sh
set -u
...
[ -x labs/m18-testing-cloud/load-test.sh ] && ok "load-test.sh present" || bad "load-test.sh missing"

echo
echo "== result: $pass passed, $fail failed =="
[ "$fail" -eq 0 ] && { echo "M18 ACCEPTANCE: PASS"; exit 0; } || { echo "M18 ACCEPTANCE: FAIL"; exit 1; }
