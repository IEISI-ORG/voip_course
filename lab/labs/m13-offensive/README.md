# Lab M13 — VoIP Threats & Offensive Testing (Authorized)
...
```bash
bash labs/m13-offensive/verify.sh        # torture the SBC; confirm it survives (no crash)
```
...
```
```bash
bash labs/m13-offensive/torture.sh 172.28.10.10   # malformed-input robustness
```
...
For each finding, capture its **detection signature** (log pattern / metric / Wazuh rule) so M15
can alert on it. Link each signature to its finding.

**Deliverable:** a detection signature per finding, carried into the M15 monitoring lab.
