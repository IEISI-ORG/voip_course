#!/usr/bin/env bash
# VoIPSec M15 acceptance test — SIP parser robustness (RFC 4475 / BF7). Fail-closed & self-
...
# Run from lab/:  bash labs/m15-offensive/verify.sh
set -u
...
if [ "$alive0" -eq 1 ]; then
  bash labs/m15-offensive/torture.sh "$SBC" >/dev/null 2>&1 || true
  sleep 1
...
[ "$fail" -eq 0 ] && { echo "M15 ACCEPTANCE: PASS"; exit 0; } || { echo "M15 ACCEPTANCE: FAIL"; exit 1; }
