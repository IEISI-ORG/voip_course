...
1. **Detect** — which alert/signature fired (link the M15 rule).
...
- **Detect:** `TollFraudSpend` (spend cap breach) / M14 fraud-detect.
...
- **Detect:** unexpected SPAN/mirror, `RecordingAccessAnomaly`, or plaintext RTP where SRTP is
  policy.
- **Contain:** enforce SRTP-only (M11); revoke exposed keys/certs; isolate the suspect host.
- **Eradicate:** rotate SRTP/TLS material; audit access; close the exposure.
- **Recover:** confirm media is encrypted end-to-end; review recordings' at-rest encryption (M14).
