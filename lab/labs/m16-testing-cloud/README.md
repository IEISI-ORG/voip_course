# Lab M16 — Testing, Interop, Automation & Cloud

**Module:** [M16](../../../course/modules/16-testing-interop-automation-cloud.md) · **Est.** 5h ·
**Prereqs:** M6–M15.
...
## Auto-graded core
```bash
bash labs/m16-testing-cloud/verify.sh        # CI valid + repo lint + no Actions injection (offline)
```
Mirrors what CI runs on every push (`.github/workflows/ci.yml`).

## Lab 16.1 — Load & regression  (30 pts)
```bash
RATE=5 CALLS=50 bash labs/m16-testing-cloud/load-test.sh
RATE=50 CALLS=500 bash labs/m16-testing-cloud/load-test.sh   # find the knee
```
