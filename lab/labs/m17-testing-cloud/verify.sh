#!/usr/bin/env bash
# VoIPSec M17 acceptance test — the automation/CI is valid and the repo passes its own lint
# (offline, deterministic, fail-closed). Mirrors what CI runs on every change.
# Run from lab/:  bash labs/m17-testing-cloud/verify.sh
set -u
...
[ -x labs/m17-testing-cloud/load-test.sh ] && ok "load-test.sh present" || bad "load-test.sh missing"
