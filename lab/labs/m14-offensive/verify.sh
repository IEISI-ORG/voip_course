#!/usr/bin/env bash
# VoIPSec M14 acceptance test — SIP parser robustness (RFC 4475 / BF7). Fail-closed & self-
...
# Run from lab/:  bash labs/m14-offensive/verify.sh
set -u
...
if [ "$alive0" -eq 1 ]; then
  bash labs/m14-offensive/torture.sh "$SBC" >/dev/null 2>&1 || true
  sleep 1
