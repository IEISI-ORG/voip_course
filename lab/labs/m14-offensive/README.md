# Lab M14 — VoIP Threats & Offensive Testing (Authorized)

**Module:** [M14](../../../course/modules/14-threats-offensive-testing.md) · **Est.** 5h ·
**Prereqs:** M1–M13.
...
```bash
bash labs/m14-offensive/verify.sh        # torture the SBC; confirm it survives (no crash)
```
...
```
```bash
bash labs/m14-offensive/torture.sh 172.28.10.10   # malformed-input robustness
```
...
For each finding, capture its **detection signature** (log pattern / metric / Wazuh rule) so M16
can alert on it. Link each signature to its finding.

**Deliverable:** a detection signature per finding, carried into the M16 monitoring lab.
