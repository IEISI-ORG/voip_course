#!/usr/bin/env bash
# VoIPSec M18 acceptance test — ENUM encoding/routing is correct (Lab 17.2). Deterministic,
# offline, fail-closed. Fax (T.38) and the CPaaS API labs are capture/config-graded.
# Run from lab/:  bash labs/m18-frontiers/verify.sh
set -u
cd "$(dirname "$0")/../.." || exit 3
TOOL=labs/m18-frontiers/enum-lookup.sh
