# Detection Signatures — M14 findings → M16 alerts (Lab 15.2)

Every threat the M14 assessment exercised must have a detection signature here, so a repeat is
seen in real time. Layer: **M** = metric (Prometheus), **L** = log/SIEM (Wazuh/Loki), **C** = CDR.

| Threat | Attack (M14) | Layer | Signature | Alert / rule |
|--------|--------------|-------|-----------|--------------|
...
| T4 | toll fraud / IRSF | C | spend cap breach; high-cost prefix burst | `TollFraudSpend` (from M15 fraud-detect) |
...
bash labs/m15-defense-fraud/fraud-detect.sh   # -> TollFraudSpend
bash labs/m14-offensive/torture.sh 172.28.10.10  # -> sanity-drop log rule
```
**Coverage gate:** an M14 finding with no detection signature is an incomplete defense.
