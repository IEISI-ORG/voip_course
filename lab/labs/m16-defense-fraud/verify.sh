#!/usr/bin/env bash
# VoIPSec M16 acceptance test — the fraud detector catches the IRSF pattern and the hardening
# checklist exists (Labs 14.1/14.2). Deterministic (offline analytics); fail-closed.
# Run from lab/:  bash labs/m16-defense-fraud/verify.sh
set -u
cd "$(dirname "$0")/../.." || exit 3
DIR=labs/m16-defense-fraud
