#!/bin/sh
# VoIPSec redteam — shared scope guard. Sourced by every helper. Refuses any target outside the
...
voipsec_guard() {
  ...
  case "$_t" in
    ...
    *)
      ...
      echo "This tooling is for the VoIPSec lab only. See /opt/redteam/AUTHORIZED_USE.md" >&2
      exit 2 ;;
  esac
}
